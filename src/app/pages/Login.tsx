...
import svgPaths from "../../imports/Html→Body-3/svg-za4tm4t01c";
import imgMascotContainerWithHandDrawnSubtleBorder from "../../imports/Html→Body-3/f615b89ccc7bca2523d65a10c064cb96f69ad8ff.png";
import imgDownload1 from "../../imports/Html→Body-3/c111dcd504f2824238d8c1eac7572a6b9dd6a855.png";
  ...
  return (
    <div className="h-[263.66px] pointer-events-none relative rounded-[24px] w-[304.004px]" data-name="Mascot Container with hand-drawn subtle border">
      <div aria-hidden="true" className="absolute inset-0 rounded-[24px]">
        ...
        <img alt="Mascote ManéGuide" className="absolute max-w-none object-cover rounded-[24px] size-full" src={imgMascotContainerWithHandDrawnSubtleBorder} />
      </div>
    ...
    <div className="bg-white h-[58px] relative rounded-[12px] shrink-0 w-[400px]" data-name="Input">
      <div className="content-stretch flex items-start justify-center overflow-clip pb-[20px] pt-[19px] px-[25px] relative rounded-[inherit] size-full">
        <input
          ...
          className="flex-[1_0_0] min-w-px bg-transparent border-0 outline-none font-['Work_Sans:Regular',sans-serif] font-normal text-[#6b7280] text-[16px] placeholder:text-[#6b7280]"
        />
      </div>
...
function Container5({ email, onEmailChange }: { email: string; onEmailChange: (value: string) => void }) {
  return (
...
function Container8({ onForgotPassword }: PasswordLabelProps) {
  return (
      ...
      <div className="flex flex-row items-center size-full">
        <div className="content-stretch flex items-center justify-between px-[8px] relative size-full">
          ...
          <button
            ...
            className="content-stretch flex flex-col items-start relative shrink-0 cursor-pointer bg-transparent border-0 p-0 hover:opacity-70 transition-opacity"
            data-name="Link"
          >
...
function Container7({ password, onPasswordChange, onForgotPassword }: Container7Props) {
  return (
    <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-[405px]" data-name="Container">
      <Container8 onForgotPassword={onForgotPassword} />
...
function Button({ onClick }: LoginButtonProps) {
  return (
    <button
      onClick={onClick}
      className="absolute bg-[#ff6b00] drop-shadow-[4px_4px_0px_#94928f] h-[69px] left-[66px] rounded-[12px] top-[220px] w-[268px] cursor-pointer border-0 hover:bg-[#e56100] transition-colors"
      data-name="Button"
    >
...
function Form({ email, password, onEmailChange, onPasswordChange, onForgotPassword, onSubmit }: FormProps) {
  return (
    <div className="absolute content-stretch flex flex-col gap-[24px] items-start left-[17.86px] top-[72.8px] w-[518px]" data-name="Form">
      <Container5 email={email} onEmailChange={onEmailChange} />
      <Container7 password={password} onPasswordChange={onPasswordChange} onForgotPassword={onForgotPassword} />
...
function Container4({ email, password, onEmailChange, onPasswordChange, onForgotPassword, onSubmit }: Container4Props) {
  return (
...
interface Container12Props {
  onAdminLogin: () => void;
}

function Container12({ onAdminLogin }: Container12Props) {
  return (
    <button
      onClick={onAdminLogin}
      className="absolute left-[-14px] top-[478px] w-[518px] bg-transparent border-0 cursor-pointer hover:opacity-70 transition-opacity"
      data-name="Container"
    >
...
function LoginFormCard({ email, password, onEmailChange, onPasswordChange, onForgotPassword, onSubmit, onAdminLogin }: LoginFormCardProps) {
  return (
    <div className="-translate-y-1/2 absolute bg-white content-stretch flex flex-col h-[525px] items-start left-[55.8%] p-[41px] right-[-7.8%] rounded-[50px] top-[calc(50%+66.9px)]" data-name="Login Form Card">
      ...
      <Container4
        ...
        onSubmit={onSubmit}
      />
      <Container12 onAdminLogin={onAdminLogin} />
    </div>
  );
}

function Container({ email, password, onEmailChange, onPasswordChange, onForgotPassword, onSubmit, onAdminLogin }: LoginFormCardProps) {
  return (
...
function MainContentArea({ email, password, onEmailChange, onPasswordChange, onForgotPassword, onSubmit, onAdminLogin }: LoginFormCardProps) {
  return (
    ...
    <div className="flex-[1_0_0] min-w-px relative" data-name="Container">
      <div className="bg-clip-padding border-0 border-[transparent] border-solid content-stretch flex flex-col items-start overflow-clip relative rounded-[inherit] size-full">
        <input
          ...
          className="w-full bg-transparent border-0 outline-none font-['Work_Sans:Regular',sans-serif] font-normal text-[#6b7280] text-[16px] placeholder:text-[#6b7280]"
        />
      </div>
    </div>
  );
  ...
  return (
    <div className="absolute content-stretch flex gap-[16px] items-center left-[288px] top-[15px]" data-name="Nav">
      <button
        ...
        className="content-stretch flex flex-col h-[33px] items-start pb-[4px] relative shrink-0 w-[86px] bg-transparent border-0 cursor-pointer hover:opacity-70 transition-opacity"
        data-name="Link"
      >
      ...
      <button
        onClick={() => navigate("/lado-b")}
        className="content-stretch flex flex-col items-start relative shrink-0 bg-transparent border-0 cursor-pointer hover:opacity-70 transition-opacity"
        data-name="Link"
      >
    ...
    <div className="absolute contents left-[152px] top-[5px]">
      <div className="absolute h-[56px] left-[152px] top-[5px] w-[57px]" data-name="download 1">
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <img alt="Logo ManéGuide" className="absolute h-[182.99%] left-[-35.65%] max-w-none top-[-29.57%] w-[179.14%]" src={imgDownload1} />
        </div>
      </div>
    </div>
  );
...
function Link4() {
  return (
    <button className="content-stretch flex flex-col items-start opacity-80 relative self-stretch shrink-0 bg-transparent border-0 cursor-pointer hover:opacity-100 transition-opacity" data-name="Link">
...
export default function Login() {
  ...
  const handleLogin = () => {
    ...
    alert(`Bem-vindo, ${email}! Redirecionando para a home...`);
    navigate("/");
  };
